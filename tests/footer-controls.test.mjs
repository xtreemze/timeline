...
test("footer controls use disclosure/group semantics without redundant toggle state", async () => {
  ...
  assert.match(index, /id="timeline-view-controls-toggle"[\s\S]*aria-expanded="false"[\s\S]*aria-label="View controls"/);
...
test("footer controls share a 44px target and explicit focus treatment", async () => {
  ...
  assert.match(
    css,
    /app-footer-context-actions\s*\{[\s\S]*overflow-x:\s*auto/,
  );
  assert.match(
    css,
