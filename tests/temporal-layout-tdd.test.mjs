...
test("#268 cluster hysteresis has separate enter and exit thresholds", async () => {
  ...
  assert.equal(
    layout.clusterMembershipWithHysteresis(false, 90, thresholds),
    false,
  );
  assert.equal(
    layout.clusterMembershipWithHysteresis(false, 79, thresholds),
    true,
  );
  assert.equal(
    layout.clusterMembershipWithHysteresis(true, 100, thresholds),
    true,
  );
  assert.equal(
    layout.clusterMembershipWithHysteresis(true, 121, thresholds),
    false,
  );
});
...
test("#268 rapid reversal inside the hysteresis band cannot flap cluster membership", async () => {
  ...
  for (const distance of [84, 92, 105, 88, 117, 96]) {
    clustered = layout.clusterMembershipWithHysteresis(
      clustered,
      distance,
      thresholds,
    );
...
test("#268 committed layout planning is deterministic for fixed inputs", async () => {
  ...
  const second = layout.planCommittedTemporalLayout(
