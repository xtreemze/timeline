...
test("WebMCP transaction editing can manage every canonical collection atomically", () => {
  ...
  const edited = webmcp.applyOperations(base, [
    ...
    { op: "upsert", collection: "entities", id: "charlie", value: { type: "person", name: "Charlie", attributes: {} } },
    { op: "upsert", collection: "relationships", id: "rel-b", value: { subjectId: "charlie", objectId: "alice", predicate: "calls", itemIds: [] } },
    { op: "patch", collection: "stories", id: "story-a", value: { description: "Updated by agent" } },
  ...
  assert.equal(edited.relationships.find((relationship) => relationship.id === "rel-b")?.subjectId, "charlie");
...
test("Memgraph export preserves Timeline action direction, predicate, namespace, and round-trip payload", () => {
  ...
  assert.ok(bundle.setupCypher.some((query) => /TimelineEntity.*timelineKey IS UNIQUE/.test(query)));
...
test("Memgraph MCP query rows import back into the matching Timeline collections", () => {
  ...
  const imported = memgraph.importSnapshot({
    entities: [{ recordJson: JSON.stringify(entity) }, { recordJson: JSON.stringify(base.entities[1]) }],
