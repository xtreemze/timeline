...
test("presentation map is semantic and selects a reasonable zoom from canonical place radius", async () => {
  ...
  assert.match(source, /doubleClickZoom:\s*this\.interactive/);
  assert.match(source, /keyboard:\s*this\.interactive/);
  ...
  assert.match(source, /accuracy <= 50\) return 16/);
  assert.match(source, /accuracy <= 250\) return 15/);
  assert.match(source, /accuracy <= 1000\) return 13/);
  assert.match(source, /accuracy <= 5000\) return 11/);
