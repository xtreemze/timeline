...
test("timeline exposes semantic zoom while retaining keyboard fit commands", async () => {
  ...
  assert.match(
    htmlSource,
    /id="timeline-zoom-level"[^>]*type="range"[^>]*min="0"[^>]*max="100"[^>]*value="100"[^>]*aria-valuetext="Focused event only"/,
  );
  ...
  assert.match(
    htmlSource,
    /id="presentation-fullscreen-toggle"[^>]*data-semantic-icon="fullscreen"/,
  );
  assert.match(
    htmlSource,
    /id="timeline-auto-toggle"[^>]*data-semantic-icon="play"[^>]*aria-label="Play slideshow"/,
  );
  assert.match(
    appSource,
    /setSemanticControlIcon\([\s\S]*playing \? "pause" : "play"[\s\S]*playing \? "Pause slideshow" : "Play slideshow"/,
  );
  assert.match(viewSource, /setSemanticZoom\(value\)/);
  assert.match(viewSource, /semanticContextItems\(item\)/);
  assert.match(
    viewSource,
    /clearTemporalAccentFromProjectHeading\(label\)[\s\S]*getBoundingClientRect[\s\S]*avoids-project-heading/,
  );
  assert.match(
    viewSource,
    /this\.orientation === "horizontal"[\s\S]*shiftRight[\s\S]*label\.style\.left[\s\S]*projectHeadingCollisionAxis = "inline"/,
  );
  assert.doesNotMatch(
    viewSource,
    /this\.orientation === "horizontal"[\s\S]{0,220}headingRect\.height \+ gap/,
  );
...
test("time labels yield to year context while year overflow stays visible", async () => {
  ...
  assert.match(viewSource, /canOverflowPrimaryEdge = \/year\/[\s\S]*!canOverflowPrimaryEdge/);
  ...
  assert.match(styles, /#app-shell #presentation-stage,[\s\S]*overflow:\s*hidden/);
});
...
test("cluster activation marks impossible all-visible temporal separation for lane expansion fallback", () => {
  ...
  const plan = clustering.clusterExpansionViewport(items, { start: 0, end: 1000 }, 600, 156);
...
test("graph semantics reject action nodes and generic association predicates", () => {
  ...
  const normalized = graph.normalizeGraphData({
    ...
    places: [
      { id: "known-place", name: "Known Place", geometry: { type: "Point", coordinates: [1, 1] } },
    ],
    relationships: [
  ...
  assert.ok(
    graph
      .validateGraphInput(selfLoopInput)
      .some((error) => /Self-loop relationships are not permitted/.test(error)),
  );
    ...
    relationships: [
      {
        ...
        objectId: "b",
        predicate: "calls",
        time: { type: "instant", start: { value: "2026-09-20T10:00Z" } },
      },
      {
        id: "r2",
        subjectId: "a",
  ...
  assert.ok(
    invalidContext.some((error) => /duplicates canonical spatiotemporal context/.test(error)),
  );
  ...
  assert.ok(
    invalidPlaces.some((error) =>
      /requires Point coordinates or Polygon\/MultiPolygon area geometry/.test(error),
    ),
  );
  ...
  assert.match(
    graph.getGraphContract().requiredAuthoringWorkflow.join(" "),
    /Extract every durable named entity/,
  );
  ...
  assert.ok(
    taxonomyErrors.some((error) =>
      /entity nodes cannot carry timeline category\/group taxonomy/.test(error),
    ),
  );
  assert.ok(
    taxonomyErrors.some((error) =>
      /graph\/place records cannot carry timeline category\/group taxonomy/.test(error),
    ),
  );
  assert.ok(
    taxonomyErrors.some((error) =>
      /relationships cannot carry timeline category\/group taxonomy/.test(error),
    ),
  );
    ...
    [
      {
        ...
        type: "person",
        name: "The Prince",
        alternateNames: ["Prince"],
        attributes: { storyId: "snow" },
      },
      {
...
test("legacy place nodes and item locations migrate into reusable edge place context", () => {
  ...
  assert.equal(
    migrated.entities.some((entity) => entity.id === "place-old"),
    false,
  );
  ...
  assert.equal(
    migrated.relationships.find((edge) => edge.id === "legacy-place-edge")?.predicate,
    "met",
  );
});
...
test("event editor persists configurable terminal and connector presentation into timeline rendering", async () => {
  ...
  assert.ok(
    app.includes("lane: Number.isInteger(item.presentation?.lane) ? item.presentation.lane : null"),
  );
  ...
  assert.ok(
    view.includes('node.dataset.laneMode = Number.isInteger(item.lane) ? "manual" : "auto"'),
  );
  ...
  assert.ok(
    styles.includes(
      '.timeline-event[data-connector-routing="orthogonal"] .timeline-event-connector-turn',
    ),
  );
  assert.ok(
    styles.includes('.timeline-event[data-connector-style="dashed"] .timeline-event-connector'),
  );
  assert.ok(
    styles.includes('.timeline-event[data-connector-style="dotted"] .timeline-event-connector'),
  );
  assert.ok(
    styles.includes(
      '.timeline-event[data-connector-endpoint="dot"] .timeline-event-connector::after',
    ),
  );
  assert.ok(
    styles.includes(
      '.timeline-event[data-connector-endpoint="arrow"] .timeline-event-connector::after',
    ),
  );
});
...
test("timeline view assigns automatic lanes and honors a persisted manual lane in both orientations", async () => {
  ...
  assert.match(
    source,
    /Number\.isInteger\(preferredLane\)[\s\S]*occupied\[preferredLane\] = position[\s\S]*return preferredLane/,
  );
...
test("timeline pinch zoom tracks two touch pointers and keeps the temporal anchor under the gesture midpoint", async () => {
  ...
  assert.match(
    source,
    /scale\.zoom\([\s\S]*this\.pinch\.viewport,[\s\S]*factor,[\s\S]*this\.pinch\.anchorTime,[\s\S]*MIN_SPAN_MS/,
  );
  assert.match(source, /event\.preventDefault\(\)/);
  assert.match(source, /const interactiveTarget = event\.target\.closest/);
  assert.match(
    source,
    /this\.touchPointers\.set\(event\.pointerId[\s\S]*if \(interactiveTarget\) return/,
  );
...
test("timeline double tap zooms toward the tapped temporal coordinate without competing with pan or pinch", async () => {
  ...
  assert.match(
    source,
    /Math\.hypot\(point\.x - previous\.x, point\.y - previous\.y\).*TOUCH_DOUBLE_TAP_DISTANCE_PX/s,
  );
...
test("mobile-first shell keeps primary controls compact and bounded", async () => {
  ...
  assert.match(
    ...
    "View uses the native invoker relationship",
  );
  assert.match(
    htmlSource,
...
test("View toolbar keeps native toggle state while measured coordinates attach it to the trigger", async () => {
  ...
  assert.match(
    appSource,
    /viewControls\?\.addEventListener\("toggle",[\s\S]*syncViewControlsChrome\(\)/,
  );
  ...
  assert.match(
    appSource,
    /workspaceToolViewport\(\)[\s\S]*preferredTop[\s\S]*fallbackTop[\s\S]*clamped-above[\s\S]*clamped-below/,
  );
  assert.match(
    appSource,
    /--view-controls-left[\s\S]*--view-controls-top[\s\S]*--view-controls-right[\s\S]*--view-controls-bottom/,
  );
  ...
  assert.match(
    appSource,
    /ResizeObserver[\s\S]*viewControlsResizeObserver[\s\S]*observe\(els\.viewControls\)[\s\S]*observe\(els\.viewControlsToggle\)/,
  );
  ...
  assert.match(
    timelineCss,
    /right:\s*var\(--view-controls-right,[\s\S]*bottom:\s*var\(--view-controls-bottom,[\s\S]*left:\s*var\(--view-controls-left, auto\)/,
  );
  assert.doesNotMatch(timelineCss, /anchor\(right\)|anchor\(top\)|position-try-fallbacks/);
  assert.match(
    timelineCss,
    /#app-shell #presentation-stage:fullscreen \.timeline-view-toolbar:popover-open[\s\S]*top:\s*max\(\.4rem[\s\S]*right:\s*max\(\.4rem/,
  );
...
test("persistent relation graph fills the stage outside chronology and stays behind focused detail", async () => {
  ...
  assert.match(
    timelineCss,
    /\.timeline-surface\.is-landscape[\s\S]*width:\s*100%[\s\S]*max-width:\s*none/,
  );
  assert.match(
    timelineCss,
    /\.timeline-surface\.is-portrait[\s\S]*height:\s*100%[\s\S]*max-height:\s*none/,
  );

  assert.match(styles, /The persistent relation graph is a stage canvas/);
  assert.match(
    styles,
    /#app-shell #presentation-stage:not\(:fullscreen\) > \.graph-lens:not\(\[hidden\]\)\s*\{[\s\S]*position:\s*absolute/,
  );
  assert.match(
    styles,
    /#app-shell #presentation-stage > \.graph-lens:not\(\[hidden\]\)\s*\{[\s\S]*margin:\s*0[\s\S]*padding:\s*0[\s\S]*scrollbar-gutter:\s*auto[\s\S]*border:\s*0[\s\S]*border-radius:\s*0/,
  );
  assert.match(
    styles,
    /\.temporal-graph-view,[\s\S]*\.temporal-graph-canvas\s*\{[\s\S]*width:\s*100%[\s\S]*height:\s*100%[\s\S]*min-width:\s*0[\s\S]*min-height:\s*0[\s\S]*margin:\s*0[\s\S]*padding:\s*0/,
  );
  assert.doesNotMatch(
    styles,
    /\.app-editor-sheet,\s*\.app-browser-sheet,\s*#app-shell #presentation-stage > \.graph-lens/,
  ...
  assert.match(
    timelineCss,
    /@media \(min-width:\s*700px\)[\s\S]*--relations-inline-rail:\s*clamp\(280px, 28dvw, 400px\)/,
  );
  ...
  assert.match(
    timelineCss,
    /data-orientation="portrait"[\s\S]*> \.graph-lens:not\(\[hidden\]\)\s*\{[\s\S]*top:\s*0;[\s\S]*right:\s*calc\(var\(--relations-inline-rail\) \+ var\(--relations-right-safe\)\);[\s\S]*bottom:\s*0;[\s\S]*left:\s*0;/,
  );
  assert.match(
    timelineCss,
    /data-orientation="portrait"\] > \.timeline-surface[\s\S]*--timeline-axis-cross:\s*68%[\s\S]*top:\s*0;[\s\S]*bottom:\s*0;[\s\S]*width:\s*var\(--relations-inline-rail\)[\s\S]*height:\s*100%/,
  );

  assert.match(
    timelineCss,
    /data-orientation="landscape"[\s\S]*> \.graph-lens:not\(\[hidden\]\)\s*\{[\s\S]*top:\s*0;[\s\S]*right:\s*0;[\s\S]*bottom:\s*calc\(var\(--relations-bottom-safe\) \+ var\(--relations-block-rail\)\);[\s\S]*left:\s*0;/,
  );
  assert.match(
    timelineCss,
    /data-orientation="landscape"\] > \.timeline-surface[\s\S]*--timeline-axis-cross:\s*42%[\s\S]*right:\s*0;[\s\S]*left:\s*0;[\s\S]*width:\s*100%[\s\S]*height:\s*var\(--relations-block-rail\)/,
  );
  assert.match(
    timelineCss,
    /@media \(min-width:\s*900px\)[\s\S]*--relations-bottom-safe:\s*max\(\.55rem/,
  );

  assert.match(
    timelineCss,
    /Mobile persistent relation composition[\s\S]*data-orientation="portrait"[\s\S]*> \.graph-lens:not\(\[hidden\]\)[\s\S]*top:\s*0;[\s\S]*bottom:\s*0;[\s\S]*left:\s*0/,
  );
  assert.match(
    timelineCss,
    /Mobile persistent relation composition[\s\S]*data-orientation="landscape"[\s\S]*> \.graph-lens:not\(\[hidden\]\)[\s\S]*top:\s*0;[\s\S]*right:\s*0;[\s\S]*left:\s*0/,
  );
  assert.doesNotMatch(timelineCss, /--mobile-top-chrome/);

  assert.match(
    timelineCss,
    /#presentation-stage:fullscreen > \.graph-lens \{\s*display:\s*block !important;/,
  );
...
test("timeline touch state recovers from lost capture, backgrounding, viewport changes, and rotation", async () => {
  ...
  assert.match(
    source,
    /document\.addEventListener\("visibilitychange"[\s\S]*document\.hidden[\s\S]*abortSurfaceGesture/,
  );
  assert.match(source, /window\.addEventListener\("orientationchange", abortSurfaceGesture\)/);
  assert.match(
    source,
    /screen\?\.orientation\?\.addEventListener\?\.\("change", abortSurfaceGesture\)/,
  );
...
test("coarse-pointer forms and compact calendars keep touch controls usable", async () => {
  ...
  assert.match(
    styles,
    /@media \(pointer: coarse\)[\s\S]*\.button,[\s\S]*\.tab[\s\S]*min-height:\s*44px/,
  );
  assert.match(
    styles,
    /@media \(pointer: coarse\)[\s\S]*\.icon-button[\s\S]*min-width:\s*44px[\s\S]*min-height:\s*44px/,
  );
  assert.match(
    styles,
    /field input:not\(\[type="color"\]\)[\s\S]*field select[\s\S]*min-height:\s*44px/,
  );
  assert.match(
    styles,
    /@media \(pointer: coarse\) and \(max-width: 380px\)[\s\S]*range-calendar[\s\S]*width:\s*calc\(100dvw - 4px\)/,
  );
...
test("phone portrait keeps a usable chronology rail and opens View controls inward", async () => {
  ...
  assert.match(
    timelineCss,
    /data-orientation="portrait"\] > \.timeline-surface[\s\S]*--timeline-axis-cross:\s*58%[\s\S]*overflow:\s*visible/,
  );
  assert.match(
    timelineCss,
    /data-orientation="portrait"\] \.timeline-tick-label[\s\S]*left:\s*auto[\s\S]*right:\s*16px[\s\S]*text-align:\s*right/,
  );
  assert.match(
    appSource,
    /orientation === "portrait"[\s\S]*preferredLeft = triggerRect\.left - toolbarWidth - gap[\s\S]*placement = "left"/,
  );
  assert.match(
    appSource,
    /orientation === "portrait"[\s\S]*top = triggerRect\.bottom - toolbarHeight/,
  );
});
...
test("empty phone portrait keeps chronology readable and project tools attached", async () => {
  ...
  assert.match(
    timelineCss,
    /data-orientation="portrait"\]\[data-empty="true"\][\s\S]*--mobile-relations-inline-rail:\s*clamp\(152px, 44dvw, 188px\)/,
  );
  assert.match(
    timelineCss,
    /timeline-surface\.is-portrait \.timeline-empty-hint[\s\S]*right:\s*calc\(100% - var\(--timeline-axis-cross\) \+ \.65rem\)[\s\S]*writing-mode:\s*vertical-rl/,
  );
  assert.match(
    timelineCss,
    /data-orientation="portrait"\]\[data-empty="true"\] > \.timeline-surface[\s\S]*--timeline-axis-cross:\s*62%[\s\S]*overflow:\s*visible/,
  );
  assert.match(
    timelineCss,
    /data-orientation="portrait"\]\[data-empty="true"\] \.timeline-project-title input[\s\S]*height:\s*clamp\(96px, 16dvh, 144px\)/,
  );
  assert.match(
    appSource,
    /workspaceToolDockResizeObserver = new ResizeObserver[\s\S]*observe\(els\.appToolDock\)[\s\S]*observe\(els\.projectMenuToggle\)/,
  );
  assert.match(
    appSource,
    /document\.fonts\?\.ready\?\.then\(\(\) => \{[\s\S]*positionWorkspaceToolDock\(\)/,
  );
});
