...
test.describe('Mobile-first Timeline layout contracts', () => {
  ...
  test('Project stays in the footer app bar and reachable while editing', async ({ page }) => {
    ...
    const dock = page.locator('.app-tool-dock');
    const projectButton = dock.locator('#project-menu-toggle');
    const editorButton = dock.locator('#editor-toggle');
    const browseButton = dock.locator('#timeline-browser-toggle');
    const viewButton = dock.locator('#timeline-view-controls-toggle');
    ...
    // Keep application chrome stable while editing. Project is required here because
    // load/import/clear project commands are intentionally edit-only.
    ...
    const menuBox = await expectInsideViewport(menu, PHONE_PORTRAIT);
    ...
    expect(menuBox.y + menuBox.height).toBeLessThanOrEqual((buttonBox?.y ?? PHONE_PORTRAIT.height) - 5);
  });
