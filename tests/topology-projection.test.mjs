...
import {
  projectFocusedTopology,
  projectTopology,
} from "../src/projection/topology-projection.ts";
...
test("topology projection consumes the explicit active occurrence relationship set", () => {
  ...
  assert.deepEqual(projection.nodes.map((node) => String(node.id)), ["a", "b", "c"]);
  assert.deepEqual(projection.edges.map((edge) => String(edge.id)), ["ab", "bc"]);
...
test("topology projection has no independent temporal parsing or activation semantics", () => {
  ...
  assert.deepEqual(projection.edges.map((edge) => String(edge.id)), ["ab"]);
});
...
test("focused topology expands only through the supplied active relationships", () => {
  ...
  assert.deepEqual(focused.edges.map((edge) => String(edge.id)), ["ab", "bc"]);
});
...
test("occurrence/item focus resolves canonical relationship context without pseudo-nodes", () => {
  ...
  assert.deepEqual(focused.nodes.map((node) => String(node.id)), ["b", "c"]);
  assert.deepEqual(focused.edges.map((edge) => String(edge.id)), ["bc"]);
});
...
test("legacy relation-change context can locate canonical edges but never enters topology", () => {
  ...
  assert.deepEqual(focused.nodes.map((node) => String(node.id)), ["a", "b"]);
  assert.deepEqual(focused.edges.map((edge) => String(edge.id)), ["ab"]);
});
