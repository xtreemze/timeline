...
test("landscape places focused detail left of graph with full-width timeline below", async ({
  ...
  expect(timelineBox.y).toBeGreaterThanOrEqual(
    Math.min(focusBox.y + focusBox.height, graphBox.y + graphBox.height) - 2,
  );
...
test("portrait stacks focused detail above graph while timeline owns the full right rail", async ({
  page,
}) => {
  const timeline = page.locator("#timeline-view");
