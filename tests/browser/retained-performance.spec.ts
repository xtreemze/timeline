...
test('retained renderer publishes phase-attributed performance evidence', async ({ page }, testInfo) => {
  ...
  const positions = Array.from({ length: 40 }, (_, index) => {
    ...
    // Traverse the scene and reverse once so p95 is based on a meaningful
    // interaction sample rather than one or two scheduler-sensitive frames.
  ...
  // Collect enough independent render samples for p95 to represent sustained
  // direct manipulation rather than a single CI/WebKit event-loop stall.
