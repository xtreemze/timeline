...
test("fullscreen keeps the timeline full-stage and moves focused detail into a responsive top layer", async () => {
  ...
  assert.match(
    timelineCss,
    /#presentation-stage:fullscreen > \.timeline-view[\s\S]*grid-column:\s*1\s*\/\s*-1\s*!important[\s\S]*grid-row:\s*1\s*\/\s*-1\s*!important/,
  );
  assert.match(
    timelineCss,
    /#presentation-stage:fullscreen > \.graph-lens,[\s\S]*\.presentation-map-panel[\s\S]*display:\s*none\s*!important/,
  );
...
test("focused Place and Relations reuse the single map and graph surfaces as interactive backdrops", async () => {
  ...
  assert.match(
    timelineCss,
    /timeline-focus-place-backdrop \.presentation-map[\s\S]*touch-action:\s*none/,
  );
  assert.match(
    timelineCss,
    /timeline-focus-section-backdrop[\s\S]*z-index:\s*1[\s\S]*pointer-events:\s*auto/,
  );
...
test("focused event detail uses a shared View Transition with its timeline terminal", async () => {
  ...
  assert.match(
    source,
    /this\.renderForViewTransition\(\);[\s\S]*returnOrigin = this\.focusTransitionOrigin\(previousId\)/,
  );
...
test("adjacent focused-event navigation finishes viewport motion before swapping shared detail", async () => {
  ...
  assert.match(
    app,
    /focusCurrentStoryItem\(Boolean\(options\.focusEvent\),\s*\{[\s\S]*direction:\s*delta < 0 \? -1 : 1/,
  );
  ...
  assert.match(
    source,
    /transitionFocusTo\(item, direction = 1\)[\s\S]*await this\.animateViewportTo\(plan\.viewport\)[\s\S]*token !== this\.focusNavigationToken/,
  );
  assert.match(
    source,
    /this\.selectedId && this\.selectedId !== id && !this\.prefersReducedMotion\(\)/,
  );
  assert.match(
    source,
    /focusTransitionTypes\(kind, direction = 0\)[\s\S]*timeline-focus-backward[\s\S]*timeline-focus-forward/,
  );
  assert.match(
    source,
    /transition\.finished\.then\(cleanupAdjacentTransition, cleanupAdjacentTransition\)/,
  );
});
...
test("timeline range bars are identifiable and labels share event color semantics", async () => {
  ...
  assert.match(
    css,
    /\.timeline-event-copy strong[\s\S]*display:\s*-webkit-box[\s\S]*-webkit-line-clamp:\s*2[\s\S]*white-space:\s*normal/,
  );
  assert.match(
    css,
    /\.timeline-event-copy span[\s\S]*text-overflow:\s*ellipsis[\s\S]*white-space:\s*nowrap/,
  );
...
test("application shell keeps the timeline viewport-owned while utility surfaces overlay it", async () => {
  ...
  assert.match(
    html,
    /class="timeline-project-heading"[\s\S]*id="story-focus"[^>]*story-focus-header/,
  );
  assert.match(
    html,
    /id="story-prev"[^>]*data-semantic-icon="chevron-left"[^>]*aria-label="Previous story event"/,
  );
  assert.match(
    html,
    /id="story-next"[^>]*data-semantic-icon="chevron-right"[^>]*aria-label="Next story event"/,
  );
  ...
  assert.match(
    styles,
    /\.app-editor-sheet \.field-grid[\s\S]*repeat\(var\(--sheet-columns\),\s*minmax\(0,\s*1fr\)\)/,
  );
  assert.match(
    styles,
    /@media \(min-width: 900px\)[\s\S]*\.app-editor-sheet,[\s\S]*width:\s*min\(390px/,
  );
  assert.match(timelineCss, /Persistent application canvas/);
  assert.match(
    timelineCss,
    /#app-shell #timeline-view[\s\S]*position:\s*absolute[\s\S]*inset:\s*0/,
  );
  assert.doesNotMatch(timelineCss, /repeat\(12,\s*minmax\(0,\s*1fr\)\)/);
  assert.match(
    timelineCss,
    /timeline-focus-view[\s\S]*grid-template-columns:\s*repeat\(6,\s*minmax\(0,\s*1fr\)\)/,
  );
  ...
  assert.match(
    timelineCss,
    /\.timeline-view\[data-orientation="portrait"\] \.story-focus-header[\s\S]*flex-direction:\s*column/,
  );
  assert.doesNotMatch(styles, /\.app-shell\.is-event-focused[\s\S]{0,220}\.story-focus/);
  assert.match(
    timelineCss,
    /\.is-portrait \.timeline-month-accent\.avoids-project-heading[\s\S]*right:\s*calc\(7px \+ var\(--timeline-project-heading-clearance, 52px\)\)/,
  );
  assert.match(
    timelineCss,
    /\.is-landscape \.timeline-month-accent\.avoids-project-heading[\s\S]*bottom:\s*8px/,
  );
  assert.doesNotMatch(
    timelineCss,
    /\.is-landscape \.timeline-month-accent\.avoids-project-heading[\s\S]{0,120}bottom:\s*calc\(/,
  );
  assert.match(
    timelineCss,
    /\.timeline-view\[data-orientation="portrait"\] \.timeline-project-title input[\s\S]*writing-mode:\s*vertical-rl/,
  );
  assert.match(
    timelineCss,
    /\.timeline-view\[data-orientation="landscape"\] \.timeline-project-heading[\s\S]*bottom:\s*max\(\.5rem, env\(safe-area-inset-bottom\)\)/,
  );
  assert.match(
    timelineCss,
    /data-timeline-orientation="horizontal"[\s\S]*\.timeline-project-heading[\s\S]*bottom:\s*max\(\.45rem, env\(safe-area-inset-bottom\)\)/,
  );
  assert.match(
    timelineCss,
    /data-graph-open="true"[\s\S]*data-orientation="landscape"[\s\S]*\.timeline-project-heading[\s\S]*bottom:\s*calc\(var\(--relations-bottom-safe\) \+ \.35rem\)/,
  );
  assert.match(
    timelineCss,
    /data-graph-open="true"[\s\S]*data-orientation="landscape"[\s\S]*\.timeline-project-heading[\s\S]*bottom:\s*calc\(var\(--mobile-bottom-chrome\) \+ \.25rem\)/,
  );
  assert.match(
    app,
    /function positionProjectMenu\(\)[\s\S]*projectMenuToggle\.getBoundingClientRect\(\)/,
  );
  ...
  assert.match(
    app,
    /importJsonTrigger\?\.addEventListener\("click",[\s\S]*importJson\?\.click\(\)/,
  );
  assert.match(
    app,
    /importInterchangeTrigger\?\.addEventListener\("click",[\s\S]*importInterchange\?\.click\(\)/,
  );
  assert.match(app, /function closeProjectMenu/);
  assert.match(
    app,
    /editorToggle\?\.addEventListener\("click",[\s\S]*setEditorSurfaceOpen\(!ui\.editorOpen\)/,
  );
  ...
  assert.match(
    html,
    /id="timeline-view-controls-toggle"[^>]*popovertarget="timeline-view-toolbar"[^>]*popovertargetaction="toggle"/,
  );
  ...
  assert.match(
    app,
    /viewControlsToggle\?\.addEventListener\("click",[\s\S]*viewControlsAreOpen\(\)[\s\S]*closeLargeUtilitySurfaces\("view"\)[\s\S]*syncApplicationSurfaces\(\)/,
  );
  assert.doesNotMatch(app, /viewControlsOpen:\s*false|ui\.viewControlsOpen/);
  assert.doesNotMatch(
    app,
    /function syncViewControlsSurface|viewControlsOpen:\s*false|ui\.viewControlsOpen/,
  );
  assert.match(
    app,
    /function positionViewControls\(\)[\s\S]*viewControlsToggle\.getBoundingClientRect\(\)[\s\S]*viewControls\.getBoundingClientRect\(\)/,
  );
  ...
  assert.match(
    timelineCss,
    /top:\s*var\(--view-controls-top, auto\)[\s\S]*left:\s*var\(--view-controls-left, auto\)/,
  );
...
test("viewing and editing are explicit mutually exclusive application modes", async () => {
  ...
  assert.match(
    styles,
    /data-mode="edit"[\s\S]*app-tool:not\(#editor-toggle\)[\s\S]*display:\s*none/,
  );
  assert.match(app, /title\.readOnly = !editing/);
  assert.match(
    app,
    /\[els\.loadSample, els\.importJsonTrigger, els\.importInterchangeTrigger, els\.clear\][\s\S]*disabled = !editing/,
  );
  ...
  assert.match(
    app,
    /deleteItemEdit\.addEventListener\("click",[\s\S]*ui\.mode !== "edit"[\s\S]*removeItem/,
  );
  assert.match(
    app,
    /els\.title\.addEventListener\("input",[\s\S]*ui\.mode !== "edit"[\s\S]*return/,
  );
  assert.match(
    app,
    /els\.clear\.addEventListener\("click",[\s\S]*ui\.mode !== "edit"[\s\S]*return/,
  );
});
...
test("focused Context belongs to Overview and interaction chrome cannot overlap either tab", async () => {
  ...
  assert.match(
    css,
    /data-active-tab="evidence"[\s\S]*timeline-focus-summary,[\s\S]*timeline-focus-place[\s\S]*display:\s*none !important/,
  );
  assert.match(
    css,
    /timeline-focus-summary[\s\S]*display:\s*flex[\s\S]*flex-direction:\s*column[\s\S]*overflow:\s*hidden/,
  );
...
test("focused popover keeps one hero composition while Overview and Evidence own distinct content", async () => {
  ...
  assert.match(
    css,
    /timeline-focus-hero[\s\S]*min-height:\s*var\(--focus-hero-block-size\) !important[\s\S]*height:\s*var\(--focus-hero-block-size\) !important[\s\S]*max-height:\s*var\(--focus-hero-block-size\) !important/,
  );
  assert.match(
    css,
    /data-active-tab="overview"[\s\S]*timeline-focus-summary[\s\S]*grid-row:\s*2 !important/,
  );
  assert.match(
    css,
    /data-active-tab="overview"[\s\S]*timeline-focus-place[\s\S]*grid-row:\s*3 !important/,
  );
  assert.match(
    css,
    /data-active-tab="evidence"[\s\S]*timeline-focus-evidence[\s\S]*grid-row:\s*2 \/ span 2 !important/,
  );
  assert.match(
    css,
    /@media \(max-width: 760px\)[\s\S]*--focus-hero-block-size:\s*clamp\(190px, 30dvh, 250px\)/,
  );
});
...
test("fullscreen graph composition follows physical orientation and preserves direct manipulation", async () => {
  ...
  assert.match(
    css,
    /data-viewport-orientation="landscape"[\s\S]*timeline-focus-relations[\s\S]*grid-row:\s*1 !important/,
  );
  assert.match(
    css,
    /data-viewport-orientation="portrait"[\s\S]*timeline-focus-relations[\s\S]*grid-column:\s*1\s*\/\s*span 3/,
  );
  assert.match(css, /timeline-focus-relations::before[\s\S]*radial-gradient/);
  assert.match(
    css,
    /timeline-focus-relations-backdrop \.temporal-graph-canvas,[\s\S]*pointer-events:\s*auto !important/,
  );
  assert.match(
    css,
    /timeline-focus-relations-backdrop \.temporal-graph-canvas[\s\S]*cursor:\s*grab/,
  );
});
...
test("fresh startup loads the storybook sample while persisted timelines retain precedence", async () => {
  ...
  assert.match(
    loadState,
    /localStorage\.getItem\(STORAGE_KEY\)[\s\S]*return normalizeTimeline\(JSON\.parse\(current\)\)/,
  );
...
test("fullscreen presentation derives timeline axis from physical viewport without persisting it", async () => {
  ...
  assert.match(
    app,
    /setOrientation\?\.\(fullscreenOrientation, \{ persist: false, focus: false \}\)/,
  );
  assert.match(app, /timelineOrientationBeforeFullscreen/);
  assert.match(
    app,
    /setOrientation\?\.\(timelineOrientationBeforeFullscreen, \{ persist: false, focus: false \}\)/,
  );
});
...
test("contextual relations docks only the timeline surface by timeline orientation", async () => {
  ...
  assert.match(
    css,
    /data-timeline-orientation="horizontal"[\s\S]*--timeline-context-edge-span:\s*clamp\(112px, 24dvh, 260px\)/,
  );
  assert.match(
    css,
    /data-timeline-orientation="vertical"[\s\S]*--timeline-context-edge-span:\s*clamp\(132px, 29dvw, 320px\)/,
  );
});
...
test("docked timeline terminals can overhang the narrow rail without leaving the viewport", async () => {
  ...
  assert.match(
    css,
    /#app-shell #timeline-view[\s\S]*position:\s*absolute[\s\S]*inset:\s*0[\s\S]*overflow:\s*hidden/,
  );
  assert.match(
    css,
    /data-timeline-orientation="vertical"\]:has\(\.timeline-focus-view:popover-open\)[\s\S]*timeline-surface[\s\S]*overflow:\s*visible/,
  );
  assert.match(
    css,
    /data-timeline-orientation="horizontal"\]:has\(\.timeline-focus-view:popover-open\)[\s\S]*timeline-surface[\s\S]*overflow:\s*visible/,
  );
});
...
test("focused popover runtime owns its final width and height budget", async () => {
  ...
  assert.match(
    source,
    /const preferredWidth = mobileFocusLayout[\s\S]*compactAvailableWidth[\s\S]*: compact[\s\S]*rawAvailableWidth[\s\S]*this\.orientation === "vertical"[\s\S]*\? \(desktop \? 620 : 520\)[\s\S]*: \(desktop \? 760 : 640\)/,
  );
  assert.match(source, /const minimumDesktopWidth = this\.orientation === "vertical" \? 480 : 560/);
  assert.match(
    source,
    /rawAvailableWidth < minimumDesktopWidth[\s\S]*unreservedWidth >= minimumDesktopWidth/,
  );
...
test("popover inset measurement ignores an undocked full-viewport timeline surface", async () => {
  ...
  assert.match(
    source,
    /const dockedRight =[\s\S]*timelineRect\.width < viewportWidth \* 0\.6[\s\S]*timelineRect\.right >= viewportWidth - FOCUS_POPOVER_MARGIN \* 2/,
  );
  assert.match(
    source,
    /const dockedBottom =[\s\S]*timelineRect\.height < viewportHeight \* 0\.6[\s\S]*timelineRect\.bottom >= viewportHeight - FOCUS_POPOVER_MARGIN \* 2/,
  );
...
test("desktop focus card preserves a stable hero while Overview gets Context and Place", async () => {
  ...
  assert.match(
    css,
    /Focused popover tab contract[\s\S]*--focus-hero-block-size:\s*clamp\(320px, 44dvh, 400px\)/,
  );
  ...
  assert.match(
    css,
    /timeline-focus-place[\s\S]*grid-row:\s*3 !important[\s\S]*min-height:\s*104px[\s\S]*max-height:\s*104px/,
  );
  assert.match(
    css,
    /timeline-focus-title[\s\S]*font-size:\s*clamp\(2\.2rem, 7cqi, 4\.6rem\)[\s\S]*line-height:\s*\.94/,
  );
  assert.match(
    css,
    /data-orientation="landscape"[\s\S]*inline-size:\s*min\(640px, calc\(100dvw - 6\.5rem\)\)/,
  );
  assert.match(
    css,
    /data-orientation="portrait"[\s\S]*inline-size:\s*min\(520px, calc\(100dvw - 7\.2rem\)\)/,
  );
});
...
test("adjacent event navigation preserves temporal context before swapping focused detail", async () => {
  ...
  assert.match(
    source,
    /this\.select\(item\.id, \{[\s\S]*preserveViewport: true[\s\S]*adjacentDirection:/,
  );
  assert.match(
    source,
    /focusAdjacent\(delta, options = \{\}\)[\s\S]*return this\.focusItem\(this\.items\[nextIndex\]\.id/,
  );
  assert.match(
    source,
...
test("timeline viewport travel is awaitable and cancellable instead of snapping", async () => {
  ...
  assert.match(
    source,
    /queueZoom\(factor, anchorRatio\)[\s\S]*this\.focusNavigationToken \+= 1[\s\S]*this\.resolveViewportAnimation\(false\)/,
  );
});
...
test("opening and adjacent focused events use distinct shared View Transition identities", async () => {
  ...
  assert.match(
    css,
    /active-view-transition-type\(timeline-focus-forward\)[\s\S]*active-view-transition-type\(timeline-landscape\)[\s\S]*view-transition-old\(timeline-event-detail-swap\)/,
  );
  assert.match(
    css,
    /active-view-transition-type\(timeline-focus-backward\)[\s\S]*active-view-transition-type\(timeline-portrait\)[\s\S]*view-transition-new\(timeline-event-detail-swap\)/,
  );
});
...
test("focus View Transitions capture chronology movement on the active timeline axis", async () => {
  ...
  assert.match(
    source,
    /document\.startViewTransition\(\{[\s\S]*update,[\s\S]*types: this\.focusTransitionTypes\(kind, direction\)/,
  );
  assert.match(source, /adjustFocusedViewport\(item, \{ immediate: captureTimeline \}\)/);
  assert.match(
    source,
    /renderForViewTransition\(\)[\s\S]*cancelAnimationFrame\(this\.renderFrame\)[\s\S]*this\.render\(\)/,
  );
  assert.match(source, /this\.surface\.style\.viewTransitionName = FOCUS_TIMELINE_TRANSITION_NAME/);
  assert.match(
    source,
    /clearFocus\(\{ deferRender: true, deferSurfaceFocus: true \}\);[\s\S]*this\.renderForViewTransition\(\);[\s\S]*returnOrigin = this\.focusTransitionOrigin\(previousId\)/,
  );

  assert.match(css, /::view-transition-group\(timeline-focus-chronology\)/);
  assert.match(
    css,
    /active-view-transition-type\(timeline-focus-open\)[\s\S]*active-view-transition-type\(timeline-landscape\)[\s\S]*timeline-chronology-open-landscape-old/,
  );
  assert.match(
    css,
    /active-view-transition-type\(timeline-focus-open\)[\s\S]*active-view-transition-type\(timeline-portrait\)[\s\S]*timeline-chronology-open-portrait-old/,
  );
...
test("Evidence tab keeps Hero but contains no Context or Place", async () => {
  ...
  assert.match(
    css,
    /timeline-focus-hero[\s\S]*height:\s*var\(--focus-hero-block-size\) !important/,
  );
});
...
test("focused popover stays opposite chronology and reserves persistent application chrome", async () => {
  ...
  assert.match(
    source,
    /this\.orientation === "vertical"[\s\S]*left = bounds\.left[\s\S]*top = mobileFocusLayout[\s\S]*bounds\.top[\s\S]*bounds\.top \+ Math\.max\(0, \(availableHeight - height\) \/ 2\)/,
  );
  assert.match(
    source,
    /else \{[\s\S]*left = mobileFocusLayout[\s\S]*bounds\.left[\s\S]*bounds\.left \+ Math\.max\(0, \(availableWidth - width\) \/ 2\)[\s\S]*top = bounds\.top/,
  );
  ...
  assert.match(
    css,
    /data-orientation="portrait"[\s\S]*inset:\s*50%[\s\S]*safe-area-inset-left[\s\S]*translateY\(-50%\)/,
  );
});
...
test("custom event-detail morph isolates application chrome from root cross-fade", async () => {
  ...
  assert.match(
    timelineCss,
    /::view-transition-old\(root\)[\s\S]*animation:\s*none !important[\s\S]*opacity:\s*0/,
  );
  assert.match(
    timelineCss,
    /::view-transition-new\(root\)[\s\S]*animation:\s*none !important[\s\S]*opacity:\s*1/,
  );
  ...
  assert.match(
    timelineCss,
    /::view-transition-old\(timeline-event-detail-shared\)[\s\S]*timeline-event-origin-out/,
  );
  assert.match(
    timelineCss,
    /::view-transition-new\(timeline-event-detail-shared\)[\s\S]*timeline-event-detail-emerge/,
  );
  ...
  assert.match(
    styles,
    /timeline-panel > \.story-focus[\s\S]*view-transition-name:\s*timeline-story-controls/,
  );
  assert.match(
    styles,
    /::view-transition-group\(timeline-command-bar\)[\s\S]*animation:\s*none !important/,
  );
});
...
test("focused popover re-clamps after late content resize", async () => {
  ...
  assert.match(
    source,
    /this\.focusView\.matches\(":popover-open"\)[\s\S]*requestAnimationFrame\(\(\) => \{[\s\S]*this\.positionFocusPopover\(\)/,
  );
  assert.match(
    source,
    /cancelAnimationFrame\(this\.focusResizeFrame\)[\s\S]*this\.focusResizeFrame = 0/,
  );
});
...
test("focus chrome remains outside tab animation and semantic panels stay bounded", async () => {
  ...
  assert.match(
    timelineCss,
    /\.timeline-view-toolbar[\s\S]*view-transition-name:\s*timeline-view-toolbar/,
  );
  assert.match(styles, /\.app-command-bar[\s\S]*view-transition-name:\s*timeline-command-bar/);
  assert.match(styles, /\.app-tool-dock[\s\S]*view-transition-name:\s*timeline-workspace-sidebar/);
  assert.match(
    timelineCss,
    /timeline-focus-view\[popover\] > :is\([\s\S]*timeline-focus-evidence[\s\S]*max-inline-size:\s*100%/,
  );
  assert.match(
    timelineCss,
    /\.timeline-focus-place,[\s\S]*\.timeline-focus-relations[\s\S]*max-block-size:\s*100%/,
  );
});
...
test("focused popover keeps deterministic tab bands and View Transition snapshots cannot capture pointer input", async () => {
  ...
  assert.match(
    css,
    /timeline-focus-view\[popover\]:popover-open[\s\S]*box-sizing:\s*border-box[\s\S]*align-content:\s*start[\s\S]*block-size:\s*auto[\s\S]*grid-auto-rows:\s*auto/,
  );
  assert.match(
    css,
    /Focused popover tab contract[\s\S]*grid-template-rows:\s*auto minmax\(0, 1fr\) 104px !important/,
  );
  assert.match(
    css,
    /timeline-focus-tabs[\s\S]*position:\s*relative[\s\S]*grid-row:\s*1 !important/,
  );
  assert.match(
    css,
    /data-active-tab="overview"[\s\S]*timeline-focus-summary[\s\S]*grid-row:\s*2 !important/,
...
test("phone focus popovers collapse to one intrinsic-safe column after the application layout contract", async () => {
  ...
  assert.match(
    css,
    /@media \(max-width: 500px\)[\s\S]*\.timeline-focus-view\[popover\][\s\S]*grid-template-columns:\s*minmax\(0, 1fr\) !important/,
  );
  assert.match(
    css,
    /\.timeline-focus-view\[popover\] > \*[\s\S]*min-inline-size:\s*0[\s\S]*max-inline-size:\s*100%/,
  );
  assert.match(
    css,
    /@media \(min-width: 501px\)[\s\S]*grid-template-columns:\s*repeat\(6, minmax\(0, 1fr\)\) !important/,
  );
});
...
test("mobile focused-event composition stays opposite chronology and keeps compact chrome", async () => {
  ...
  assert.match(
    css,
    /timeline-focus-summary \.timeline-focus-actions[\s\S]*grid-template-columns:\s*repeat\(3, minmax\(0, 1fr\)\)/,
  );
  assert.match(
    css,
    /:is\(\.timeline-focus-place, \.timeline-focus-relations\) \.timeline-focus-section-content[\s\S]*width:\s*min\(60%, 17rem\)/,
  );
  ...
  assert.match(
    source,
    /mobileFocusLayout = bounds\.viewportWidth <= 699[\s\S]*mobileFocusRail = mobileFocusLayout[\s\S]*clamp\(bounds\.viewportWidth \* 0\.38, 136, 168\)/,
  );
  assert.match(
    source,
    /compactAvailableWidth = mobileFocusLayout && this\.orientation === "vertical"[\s\S]*rawAvailableWidth - mobileFocusRail - mobileFocusGap/,
  );
  assert.match(
    source,
    /compactAvailableHeight = mobileFocusLayout && this\.orientation === "horizontal"[\s\S]*availableHeight - mobileFocusRail - mobileFocusGap/,
  );
