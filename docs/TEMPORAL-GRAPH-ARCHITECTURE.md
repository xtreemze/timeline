...
Event narrative and graph topology must stay semantically complete. If an existing canonical entity name or alternate name appears in an event title, description, image alt text, or the free-text note of evidence attached to that event, that entity must participate as a source or target of at least one concrete action edge linked to the event through `relationship.itemIds[]` or the event's `relationChanges[]`.

This is a graph-integrity rule, not entity extraction by guesswork. Free text does not create nodes automatically. When prose introduces a person, organization, group, object, document, account, device, or other entity that is not yet canonical, the author must first create/reuse that entity node and then connect it with the action that makes the entity relevant to the event. Evidence source metadata such as title, publisher/source name, URL, file name, exhibit identifiers, semantic tags, and media credit/provenance captions are excluded because they can identify provenance rather than event participants; an evidence record's authored `note` and an image's descriptive alt text are included because they describe event context.
