...
- On constrained screens, Browse and Edit are fixed, safe-area-aware panels bounded by the Visual Viewport and the persistent application chrome: the bottom footer in landscape or the right-edge rail in portrait. Their content scrolls internally; the panel itself must never extend beyond the reachable viewport.
...
- Project/Edit/Browse/View and the project title are fused into one persistent application-chrome shell in both view and edit modes. In landscape, the four actions form the upper footer row and the project-title field sits directly beneath them while the timeline ends above the shell. In portrait, the same shell rotates into a vertical rail on the right edge: actions occupy the inner column and project identity occupies the outer title strip while the timeline ends to its left. The shell is never overlaid on chronology. Project and View popovers open inward—above in landscape, left in portrait—and remain clamped to the Visual Viewport.
...
- Project identity belongs to the common application-chrome shell rather than timeline-local chrome. In landscape it is the lower footer row; in portrait it is the outer strip of the right rail. Its title remains readable and editable only when edit state permits. The native grouped Project popover opens toward workspace content, remains in the top layer above any open editor surface, is constrained to the current Visual Viewport, and scrolls internally when available room is limited. Every Project menu command—load example, import, export, source navigation and clear project—remains available in both view and edit modes; destructive replacement/clear flows retain their explicit confirmation safeguards. Import actions remain explicit buttons wired to hidden file inputs so every visible enabled menu command is keyboard-operable.
...
Fullscreen targets `#presentation-stage`. The fused application-chrome shell is temporarily reparented into that fullscreen subtree so Project/Edit/Browse/View plus project identity remain available without changing edge ownership; editor/browser sheets remain outside fullscreen. Fullscreen reserves the same measured bottom block in landscape and right-side inline extent in portrait as ordinary application mode.
...
A dedicated application-chrome `ResizeObserver` measures the fused shell against the live Visual Viewport. Landscape writes `--workspace-footer-reserved-block`; portrait writes `--workspace-footer-reserved-inline`. The presentation stage, utility sheets, story controls, graph, map, and fullscreen composition end at that measured boundary rather than extending behind chrome. Reservations are recomputed after orientation changes, font loading, viewport resize/scroll, fullscreen reparenting, and any shell-size change.
