...
- Project/Edit/Browse/View and the project title are fused into one persistent bottom footer at every viewport size and in both view and edit modes. The four actions form the upper footer row and the project-title field forms the row directly beneath them. Timeline orientation never moves or splits that footer. Project and View popovers open inward above the footer and remain clamped to the Visual Viewport.
...
- Project identity is the lower row of the common footer rather than timeline-local chrome. Its title remains readable and editable only when edit state permits, while the Project action stays in the action row above it. The native grouped Project popover opens inward above the footer, remains in the top layer above any open editor surface, is constrained to the current Visual Viewport, and scrolls internally when vertical room is limited. Every Project menu command—load example, import, export, source navigation and clear project—remains available in both view and edit modes; destructive replacement/clear flows retain their explicit confirmation safeguards. Import actions remain explicit buttons wired to hidden file inputs so every visible enabled menu command is keyboard-operable.
...
Fullscreen targets `#presentation-stage`. The fused footer is temporarily reparented into that fullscreen subtree so Project/Edit/Browse/View plus project identity remain available without changing position; editor/browser sheets remain outside fullscreen. The stage reserves the measured footer block in fullscreen exactly as it does in ordinary application mode.
...
A dedicated footer `ResizeObserver` measures the fused footer and writes `--workspace-footer-reserved-block` from its live Visual Viewport geometry. The ordinary presentation stage ends at that reserved boundary rather than extending behind application chrome; utility sheets and story controls use the same boundary. This reservation is orientation-independent and is recomputed after font loading, viewport resize/scroll, fullscreen reparenting, and any footer-size change.
