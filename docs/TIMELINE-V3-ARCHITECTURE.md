...
- The global viewing dock is limited to Edit, Browse and View, using semantic icon + text pairs for recognition at touch and desktop distances.
- Floating workspace chrome follows timeline orientation rather than viewport width: landscape timelines deploy the global dock and expanded View controls vertically along screen edges, while portrait timelines deploy them horizontally along the bottom. Orientation changes must recompose this chrome immediately without changing control state.
