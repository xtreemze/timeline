...
Pointer Events are the baseline for Lūm-owned direct manipulation. The shared policy in `src/interaction/surface-input-policy.ts` defines primary-pointer eligibility, activation, camera-navigation keyboard commands for custom surfaces, and cursor intent. Renderer-native controllers still own their camera mechanics: deck.gl 9.4/mjolnir owns world pan/rotate/pinch/wheel/keyboard camera input, Leaflet owns map pinch/wheel/keyboard zoom/navigation, the retained timeline owns its weighted temporal camera, and the legacy Orb/D3 graph keeps its existing weighted camera and force/node-drag mechanics. deck forwards native interaction state into the shared lifecycle; Leaflet arbitrates ownership before native handlers run; Orb/D3 promotes the same graph epoch between tap, pan, pinch, and node-drag intent. These adapters report ownership without reimplementing renderer-native camera mechanics.
