...
## Public Streamable HTTP generation endpoint

For broad provider interoperability and public plugin deployment, Lūm also includes a stateless Streamable HTTP MCP service at `POST /mcp`. It is intentionally separate from the live browser authoring surface: it exposes source-to-story guidance and structural preflight, not direct access to a user's local project.

The public tools are `lum.get_story_authoring_guide` and `lum.stage_story_project`. Compatible hosts read uploaded documents themselves, construct a complete project proposal, and send only structured project/source-manifest data to the service. Final acceptance still occurs in the live application through `timeline.audit_graph`, `timeline.validate_project`, and user review.

Reference deployment, Cloudflare Worker configuration, OpenAI domain verification, portable skill guidance, and the full trust boundary are documented in [PUBLIC-MCP-STORY-GENERATION.md](PUBLIC-MCP-STORY-GENERATION.md).
