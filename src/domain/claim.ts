...
export function validateFragment(fragment: ExtractedFragment): string[] {
  ...
  if (!fragment.text.trim()) {
    findings.push("Extracted fragment text must not be empty.");
  }
...
export function validateCandidateClaim(claim: CandidateClaim, ledger: ClaimLedger): string[] {
  ...
  if (!claim.predicate.trim()) {
    findings.push("Candidate claim requires an action predicate.");
  }
  if (claim.fragmentIds.length === 0) {
    findings.push("Candidate claim requires at least one source fragment.");
  }
  if (claim.sourceIds.length === 0) {
    findings.push("Candidate claim requires at least one source.");
  }
