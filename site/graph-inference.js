(() => {
  ...
  function uniqueTextList(value, maxItems = 24, maxLength = 240) {
  ...
  function entityLookup(entities, storyIds) {
    ...
    return (name, alternateNames = []) => {
      ...
      if (stories.size) {
        const scoped = candidates.filter((entity) => stories.has(String(entity?.attributes?.storyId || "")));
  ...
  function coordinatesExplicitInSources(candidate, fragments, sourceRefs) {
    if (
    ...
    const matchesNumber = (value, target) => Math.abs(value - target) <= 1e-7;
  ...
  function placeLookup(places) {
    ...
    return (candidate) => {
      ...
      const matches = records.filter((place) => {
  ...
  function reconcileProposal(raw, context = {}, dependencies = {}) {
    const graph = dependencies.graph || globalThis.TimelineGraph;
    const spatial = dependencies.spatial || globalThis.TimelineSpatial;
    const idFactory = dependencies.idFactory || ((prefix) => `${prefix}-inferred-${Math.random().toString(36).slice(2, 10)}`);
    ...
    const allowedSourceRefs = new Set(
      normalizeFragments(context.fragments).map((fragment) => fragment.ref)
    );
    ...
    for (const candidate of Array.isArray(raw?.entities) ? raw.entities : []) {
      ...
      const normalized = {
        ...
        sourceRefs: uniqueTextList(candidate?.sourceRefs, 16, 160).filter((ref) => allowedSourceRefs.has(ref)),
    ...
    for (const candidate of Array.isArray(raw?.relationships) ? raw.relationships : []) {
      ...
      const allowedRelationshipSourceRefs = uniqueTextList(candidate?.sourceRefs, 16, 160)
        .filter((ref) => allowedSourceRefs.has(ref));
      ...
      const status = duplicate ? "merge" : mirrored ? "mirrored" : "new";
      relationships.push({
        key,
    ...
    const unresolved = [
      ...(Array.isArray(raw?.unresolved) ? raw.unresolved : []).map((entry) => ({
        kind: text(entry?.kind, 40) || "relationship",
        label: text(entry?.label, 200),
        reason: text(entry?.reason, 500),
        sourceRefs: uniqueTextList(entry?.sourceRefs, 16, 160).filter((ref) => allowedSourceRefs.has(ref))
      })).filter((entry) => entry.label || entry.reason),
      ...rejected
    ];
  ...
  function sourceEvidenceIds(sourceRefs) {
    const ids = [];
  ...
  function applyProposal(project, item, proposal, selectedRelationshipKeys = [], dependencies = {}) {
    const graph = dependencies.graph || globalThis.TimelineGraph;
    ...
    const placeByKey = new Map((proposal?.places || []).map((candidate) => [candidate.key, candidate]));
    const requiredEntityKeys = new Set();
    const requiredPlaceKeys = new Set();

    const selectedRelationships = (proposal?.relationships || []).filter((candidate) => {
    ...
    for (const key of requiredEntityKeys) {
      ...
      if (candidate.status === "new" && candidate.record) {
        if (!draft.entities.some((entity) => String(entity.id) === String(candidate.record.id))) {
    ...
    for (const key of requiredPlaceKeys) {
      ...
      if (candidate.status === "new" && candidate.record) {
        if (!draft.places.some((place) => String(place.id) === String(candidate.record.id))) {
    ...
    for (const candidate of selectedRelationships) {
      ...
      if (candidate.status === "merge" && candidate.mergeIntoRelationshipId) {
        const index = draft.relationships.findIndex(
          (relationship) => String(relationship.id) === String(candidate.mergeIntoRelationshipId)
        );
      ...
      if (!draft.relationships.some((current) => String(current.id) === String(relationship.id))) {
  ...
  globalThis.TimelineGraphInference = Object.freeze({
    SCHEMA_VERSION,
    SYSTEM_PROMPT,
    responseSchema,
    inferenceInput,
