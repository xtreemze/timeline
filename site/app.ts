...
function positionViewControls() {
      ...
      ? [
          {
            ...
            rect: {
              ...
              height: Math.min(measuredHeight, Math.max(1, viewport.height - edge * 2)),
            },
          },
        ]
      : [
  ...
  const boundedLeft =
    orientation === "portrait"
      ? Math.min(maxLeft, Math.max(minLeft, dockRect.left - gap - actualInlineSize))
      : Math.min(maxLeft, Math.max(minLeft, anchor.x - actualInlineSize / 2));
  const boundedTop =
    orientation === "portrait"
      ? Math.min(maxTop, Math.max(minTop, anchor.y - actualBlockSize / 2))
      : Math.min(maxTop, Math.max(minTop, dockRect.top - gap - actualBlockSize));
