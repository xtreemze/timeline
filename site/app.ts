...
function positionViewControls() {
      ...
      ? [
          ...
          },
          {
            ...
            rect: {
              ...
              width: Math.min(
                measuredWidth,
                Math.max(1, viewport.left + viewport.width - edge - dockRect.right - gap),
              ),
              height: measuredHeight,
            },
          },
      ...
      : [
          ...
          },
          {
            ...
            rect: {
              ...
              height: Math.min(
                measuredHeight,
                Math.max(1, viewport.top + viewport.height - edge - dockRect.bottom - gap),
              ),
            },
          },
        ];
  ...
  els.viewControls.style.setProperty("--view-controls-left", `${Math.round(selected.rect.x)}px`);
  els.viewControls.style.setProperty("--view-controls-top", `${Math.round(selected.rect.y)}px`);
...
function removeStory(id) {
  const story = getStory(id);
  if (
    !(story && window.confirm(`Delete story “${story.title}”? Timeline items will not be deleted.`))
  )
    return;
...
function ensurePresentationFocus() {
  ...
  return timelineView?.focusAdjacent(1);
}
...
function advancePresentation(delta: number, options: { focusEvent?: boolean } = {}): boolean {
  ...
  return timelineView?.focusAdjacent(delta);
}
...
function handlePresentationCommand(command: string, meta: PresentationCommandMeta = {}): boolean {
  ...
  if (command === "toggle-auto") {
    if (
      (!navigationController.auto.running || navigationController.auto.paused) &&
      !ensurePresentationFocus()
    ) {
      showStatus("No visible timeline items to present.");
      return true;
    }
