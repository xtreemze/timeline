...
function positionViewControls() {
  ...
  const maxLeft = Math.max(minLeft, viewport.left + viewport.width - edge - boundedInlineSize);
  const maxTop = Math.max(minTop, viewport.top + viewport.height - edge - boundedBlockSize);
