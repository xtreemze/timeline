(() => {
  ...
  const evidenceExtraction = globalThis.TimelineEvidenceExtraction;
  ...
  const graphInference = globalThis.TimelineGraphInference;
  ...
  if (!evidenceExtraction) throw new Error("TimelineEvidenceExtraction must load before app.js.");
  ...
  if (!graphInference) throw new Error("TimelineGraphInference must load before app.js.");
  ...
  function evidenceExtractionLabel(extraction) {
    ...
    if (extraction.unresolved?.length) parts.push(`${extraction.unresolved.length} unresolved`);
    return parts.join(" · ");
  }
  ...
  function renderEvidenceExtraction(parts, extraction) {
    ...
    for (const segment of extraction?.segments || []) {
      const locator = segment.locator?.kind === "page"
    ...
    for (const unresolved of extraction?.unresolved || []) {
      const locator = unresolved.locator?.kind === "page"
  ...
  async function evidenceBlobForRow(row) {
    ...
    return evidenceStore.getBlob(existing.file.blobKey);
  }
  ...
  async function extractEvidenceRow(row, { quiet = false } = {}) {
    ...
    try {
      const extraction = await evidenceExtraction.extract(blob, {
        mimeType: blob.type,
        fileName: blob.name || state.evidence.find((record) => record.id === id)?.file?.name || "",
        onProgress(progress) {
  ...
  async function collectEvidenceForm() {
    ...
    for (const row of els.itemEvidenceRows) {
      ...
      if (file) {
        ...
        const sizeLimit = file.type === "application/pdf" || /\.pdf$/i.test(file.name) ? 25_000_000 : 15_000_000;
        if (file.size > sizeLimit) {
          throw new Error(`Evidence files are limited to ${Math.round(sizeLimit / 1_000_000)} MB each for this format.`);
        }
      ...
      const extraction =
        evidenceExtractionDrafts.get(id) ||
        (file ? null : existing?.extraction || null);
  ...
  function ensureInferenceEvidenceIds() {
    for (const row of els.itemEvidenceRows) {
      ...
      const hasSemanticContent =
        ...
        Boolean(parts.id.value && state.evidence.find((record) => record.id === parts.id.value)?.file?.blobKey) ||
  ...
  function inferenceInputFromForm({ ensureIds = true } = {}) {
    ...
    els.itemEvidenceRows.forEach((row) => {
      ...
      const title = parts.title.value.trim();
      if (!id || !title) return;
      const note = parts.note.value.trim();
      if (note) addFragment(`evidence:${id}:note`, "evidence-note", note);
    ...
    const locationParts = [
      ...
      els.itemLocationIdentifier.value.trim() ? `Geographic identifier: ${els.itemLocationIdentifier.value.trim()}` : "",
    ...
    return {
      graphContractVersion: graph.GRAPH_CONTRACT_VERSION || graph.getGraphContract?.().version || "unknown",
      event: {
        id: itemId,
  ...
  function renderInferenceDraft() {
    ...
    if (proposal.places.length) {
      ...
      for (const candidate of proposal.places) {
        ...
        title.textContent = candidate.name;
        const status =
          candidate.status === "existing" ? "Existing place" :
          candidate.status === "new" ? "New place with explicit coordinates" :
    ...
    if (proposal.relationships.length) {
      ...
      elements.push(heading);
      const entityByKey = new Map(proposal.entities.map((candidate) => [candidate.key, candidate]));
      for (const candidate of proposal.relationships) {
        ...
        const status =
          candidate.status === "merge" ? "Merge context into existing action" :
          candidate.status === "mirrored" ? "Rejected mirrored action" :
          "New action";
        copy.append(
          title,
          inferenceMeta([status, `${Math.round(candidate.confidence * 100)}% confidence`])
    ...
    els.itemInferenceDetails.open = true;
    els.itemInferenceClear.hidden = false;
  }

  function selectedInferenceRelationshipKeys() {
    return [...(els.itemInferenceResults?.querySelectorAll('input[data-inference-relationship]:checked') || [])]
  ...
  async function runItemInference() {
    ...
    if (!input.fragments.length) {
      setError(els.itemFormError, "Add a title, description, image description, evidence note/file, or explicit location before inference.");
      return;
    }
    ...
    try {
      const raw = await graphInference.infer(input, {
        ...
        onDownloadProgress(progress) {
          inferenceStatus(`Downloading built-in model… ${Math.round(progress * 100)}%`, "downloading");
        }
      });
      ...
      inferenceStatus(error instanceof Error ? error.message : "Built-in graph inference failed.", "error");
    } finally {
      inferenceAbortController = null;
      const current = await graphInference.availability();
      els.itemInferenceRun.disabled = !current.available;
    }
  }

  function markInferenceStale() {
    if (!itemInferenceDraft || !els.itemInferenceDetails) return;
    els.itemInferenceDetails.dataset.stale = "true";
    inferenceStatus("Event context changed after inference. Rerun or clear inference before saving inferred graph facts.", "stale");
  }
  ...
  function toMarkdown() {
    ...
    if (state.evidence.length) {
      ...
      for (const record of state.evidence) {
        ...
        if (record.file?.name) lines.push(`Local attachment metadata: ${record.file.name} (${record.file.size || 0} bytes)  `);
  ...
  for (const row of els.itemEvidenceRows) {
    ...
    parts.file?.addEventListener("change", () => {
      ...
      if (file) {
        if (!supportedEvidenceFile(file)) {
          setError(els.itemFormError, "Evidence uploads must be PDF or image files.");
          parts.file.value = "";
          return;
        }
        if (!parts.id.value) parts.id.value = newId("evidence");
        if (file.type.startsWith("image/")) parts.type.value = "image";
        else if (file.type === "application/pdf" || /\.pdf$/i.test(file.name)) parts.type.value = "pdf";
        evidenceExtractionDrafts.delete(parts.id.value);
        renderEvidenceExtraction(parts, null);
        parts.fileStatus.textContent = `Selected: ${file.name}`;
      }
      markInferenceStale();
    });
  }
  ...
  els.itemForm.addEventListener("input", (event) => {
    if (event.target?.closest?.("#item-inference-details")) return;
    markInferenceStale();
  });
  ...
  els.itemForm.addEventListener("submit", async (event) => {
    ...
    try {
      if (itemInferenceDraft) {
        ...
        if (itemInferenceDraft.graphContractVersion !== (graph.GRAPH_CONTRACT_VERSION || currentInput.graphContractVersion)) {
          throw new Error("The graph contract changed after inference. Rerun inference before saving.");
        }
        if (itemInferenceDraft.fingerprint !== currentFingerprint) {
          throw new Error("Event context changed after inference. Rerun or clear inference before saving inferred graph facts.");
        }
      ...
      setError(
        els.itemFormError,
        error instanceof Error ? error.message : "The event or inferred graph facts violate the graph contract."
      );
      return;
    }
  ...
  els.timelineViewRoot.addEventListener("timelineevidenceopen", async (event) => {
    ...
    try {
      ...
      showStatus("Could not open the local evidence attachment.");
    }
  });
