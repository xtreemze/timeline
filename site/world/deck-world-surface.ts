...
export class DeckWorldSurface implements WorldSurface {
  ...
  #viewportGraphRadiusLimitPx(): number {
    ...
    // Keep the local graph inside ~84% of the short viewport dimension while
    // retaining a useful minimum on synthetic/non-DOM test containers.
    return Math.max(
      120,
      Math.min(WORLD_LOCAL_GRAPH_RADIUS_PX * 2, shortSide * 0.42),
    );
  }
        ...
        ? [
            this.#runtime.createIconLayer({
              ...
              getColor: (datum: DeckWorldEntityDatum) => {
                ...
                // Lower alpha composites the default fill too far toward paper.
