...
function uniqueTextList(value: unknown, maxItems = 24, maxLength = 240): string[] {
...
export function applyProposal(
  ...
  for (const key of requiredEntityKeys) {
    ...
    if (
      ...
      record &&
      !draft.entities.some((entity: any) => String(entity.id) === String(record.id))
    ) {
  ...
  for (const key of requiredPlaceKeys) {
    ...
    if (
      candidate.status === "new" &&
      record &&
      !draft.places.some((place: any) => String(place.id) === String(record.id))
    ) {
