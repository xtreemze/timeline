(() => {
  ...
  function normalizeGraphData(
    input,
    temporal = globalThis.TimelineTemporal,
    spatial = globalThis.TimelineSpatial,
  ) {
    const entities = (Array.isArray(input?.entities) ? input.entities : [])
      .map(normalizeEntity)
      .filter(Boolean);
  ...
  function validateGraphInput(input, spatial = globalThis.TimelineSpatial) {
    ...
    rawPlaces.forEach((raw, index) => {
      ...
      if (
        icon &&
        Array.isArray(spatial?.PLACE_ICON_NAMES) &&
        !spatial.PLACE_ICON_NAMES.includes(icon)
      ) {
        errors.push(
          `${label}: unsupported semantic icon “${icon}”. Choose a registered semantic icon.`,
        );
      }
      const markerShape = text(
        raw.markerShape || raw.marker?.shape || raw.attributes?.markerShape,
        24,
      );
      if (
        markerShape &&
        Array.isArray(spatial?.PLACE_MARKER_SHAPES) &&
        !spatial.PLACE_MARKER_SHAPES.includes(markerShape)
      ) {
    ...
    rawEntities.forEach((raw, index) => {
      ...
      if (id && itemIds.has(id))
        errors.push(`${label}: entity IDs cannot collide with chronology item IDs.`);
      if (id && storyIds.has(id))
        errors.push(`${label}: entity IDs cannot collide with story IDs.`);
    });
    ...
    rawRelationships.forEach((raw, index) => {
      ...
      if (taxonomyKeys.length) {
        errors.push(
          `${label}: relationships cannot carry timeline category/group taxonomy (${taxonomyKeys.join(", ")}). Categories classify chronology items only.`,
        );
      }
      ...
      for (const place of rawPlaces) {
        ...
        if (placeName && placeName.length >= 4 && predicateKey.includes(placeName)) {
          errors.push(
            `${label}: predicate must not embed place name “${place.name}”; use placeId.`,
          );
          break;
        }
      }
      ...
      if (subjectId && objectId && subjectId === objectId) {
        errors.push(
          `${label}: source and target must be different entity nodes. Self-loop relationships are not permitted.`,
        );
      }
      ...
      if (placeId && !placeIds.has(placeId))
        errors.push(`${label}: placeId must reference one canonical place record.`);
      ...
      if (duplicateKeys.length) {
        errors.push(
          `${label}: generic edge attributes duplicates canonical spatiotemporal context (${duplicateKeys.join(", ")}). Use edge.time and edge.placeId.`,
        );
      }
      ...
      for (const itemId of Array.isArray(raw.itemIds) ? raw.itemIds : []) {
        ...
        if (!itemIds.has(contextId)) {
          errors.push(
            `${label}: itemIds must reference chronology records, not create graph endpoints.`,
          );
    ...
    for (const ids of audit.duplicateFactGroups) {
      errors.push(
        `Edges ${ids.join(", ")} duplicate one directed action fact (same source, action, target, and temporal extent). Keep one canonical edge and merge itemIds, sourceIds, place, confidence, and other properties onto it.`,
      );
    }
    for (const [first, second] of audit.mirroredFactPairs) {
      errors.push(
        `Edges ${first} and ${second} mirror the same action and temporal extent in opposite directions. Do not create a reverse copy to simulate bidirectionality; add a reverse edge only for a genuinely distinct reverse action.`,
      );
    }
    for (const id of audit.orphanEntityIds) {
      errors.push(
        `Node ${id}: canonical entity is orphaned. Navigation containers, stories, categories, roles, and labels stay outside graph topology; a durable entity belongs in the graph only when it participates in at least one meaningful action edge.`,
      );
    }
  ...
  function migrateLegacySpatialModel(input, spatial = globalThis.TimelineSpatial) {
    ...
    migrated.relationships = (Array.isArray(migrated.relationships) ? migrated.relationships : [])
      .map((raw) => {
        ...
        if (usedPlaceEndpoint)
          relationship.predicate = stripLegacySpatialPredicate(
            relationship.predicate || relationship.label || relationship.type,
          );
    ...
    const edges = graphData.edges
      .map((edge) => {
        ...
        const structurallyTimeless =
          !relationship.time && state.changes.length === 0 && state.active;
  ...
  function neighborhoodGraph(input, rootId, viewport, { depth = 1, limit = 36 } = {}) {
    ...
    const rootChangeIds = new Set(
      normalizeRelationChanges(rootItem?.relationChanges).map((change) =>
        String(change.relationshipId),
      ),
    );
    const contextEdges = data.edges.filter(
      (edge) =>
        (Array.isArray(edge.properties?.itemIds) &&
          edge.properties.itemIds.some((id) => String(id) === root)) ||
